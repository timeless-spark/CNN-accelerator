...
res_load = torch.load("./saved_models/ex2_res.pth")

results_nano_resnet = res_load['nano_resnet']
results_micro_resnet = res_load['micro_resnet']
...
for res in results_nano_resnet:
    bs.append(res[0])
    lr.append(res[1])
    ta.append(res[2])
...
for res in results_micro_resnet:
    bs.append(res[0])
    lr.append(res[1])
    ta.append(res[2])
    l.append(res[3].cpu().detach().numpy())
...
plt.figure(1)
plt.plot(x, y1, '-o', label='nano_resnet')
plt.plot(x, y2, '-o', label='micro_resnet')
plt.plot(x, y3, '-o', label='custom_2')
